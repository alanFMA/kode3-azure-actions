...
import { AzureRepoService } from '../domain';

export default async function createRouter(
  { logger, config, identity }: Pick<PluginEnvironment, 'logger' | 'config' | 'identity'>
): Promise<express.Router> {

  const pat = config.get<BackstageConfigIntegrations>('integrations')?.azure?.[0]?.token
  const defaultOrg = config.get<BackstageConfigIntegrations>('integrations')?.azure?.[0]?.organization ?? ''
  ...
  const serviceConnectionService = new AzureServiceConnectionService(axiosService)
  ...
  router.get(
    ...
    async (_, res) => {
      logger.info(`${_.method} ${_.url}`);
      const { params: { organization } } = _
      const { locals: { userIdentity }} = res

      const userEntity = parseEntityRef(userIdentity?.identity.userEntityRef , {defaultKind: 'z', defaultNamespace: 'z'})

      const entitlements = await entitlementsService.userEntitlements({organization, select: 'Projects', $filter: `name eq '${userEntity.name}'`})
      try {
        res.json(entitlements.data.members[0].projectEntitlements
          .map(({projectRef: {id, name}}) => ({
            projectName: name,
            projectId: id,
          })))
      } catch (error) {
  ...
  router.get(
    ...
    async (_, res) => {
      ...
      const entitlements = await serviceConnectionService.list({organization, project }, query)
      try {
  ...
  router.get(
    ...
    async (_, res) => {
      ...
      const repositories = await repositoryService.list({organization, project })
      try {
